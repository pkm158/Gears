/************************************************************************/
/* Author: Peter Pon, found it online, modified by Kemi Peng				
...
public:
	...
	{
		float coscos = cosf(rad);
		float sinsin = sinf(rad);

		float fx=x*coscos-y*sinsin;
		float fy=x*sinsin+y*coscos;
	...
	void RotateY(float rad)
	{
		...
		float fx=x*coscos-(-z)*sinsin;
		float fy=x*sinsin+(-z)*coscos;
	...
	void RotateX(float rad)
	{
		...
		float fx = y*coscos-(-z)*sinsin;
		float fy = y*sinsin+(-z)*coscos;
...
inline float VectorDotf(const Vector4CPU &a, const Vector4CPU &b) 
{
...
inline Vector4CPU VectorDot(const Vector4CPU &a, const Vector4CPU &b)
{
	Vector4CPU c = VectorDotf(a,b);
	return c;
}

inline Vector4CPU Vector3CrossProduct(const Vector4CPU &a, Vector4CPU &b)
{
	Vector4CPU c;
