...
bool CGutModel::LoadMaterial_ASCII(void)
{
	...
	if ( m_iNumMaterials )
	{
		...
		for ( int i=0; i<m_iNumMaterials; i++ )
		{
			...
			/************************************************************************/
			/*         by kemi, do not use cubmap by now                                                             */
			/************************************************************************/

			fgets(szBuffer, 128, m_pFile); // environmentMap
			sscanf(szBuffer, "%s %s %s %s %s %d", szTag, szTag, szContent, szTag, szTag, &pMtl->m_MapChannel[2]);
			// eliminate '\"'
...
bool CGutModel::LoadExternTexture( int channel, const char* filename )
{
	if (NULL == filename)
	{
		return false;
	...
	for ( int i=0; i<m_iNumMaterials; i++ )
	{
		...
		strcpy(pMtl->m_szTexture[channel], filename);
	}
	return true;
}
